...
•차수(degree) : 노드가 가지고 있는 간선의 개수
-노드의 차수 : 연결된 자식 노드의 수
 A의 차수=3, B의 차수=2, C의 차수=1
-트리의 차수 : 트리에 있는 노드의 차수중 가장 큰 값
 트리A의 차수=3
...
•트리의 종류
1) 완전 트리 : 한쪽부터 포화로 되어가고 있는 트리
2) 포화 트리 : 모든 노드가 자식노드를 둘씩 가지고 있는 트리
3) 편향 트리 : 한쪽으로만 가진 형태
...
1) 노드 수= 간선 수 + 1
2) 노드 수와 높이의 관계
(최대)노드수 = 2^(높이 +1) -1  ********* 시험문제

ex 1) 노드수가 100개일때의 높이(h)는?
ex 2) 높이가 5인 이진트리가 갖는 최대 노드 수?
...
조건
- 중복을 허용하지 않음
- 루트를 기준으로 작은 값은 왼쪽, 큰 값은 오른쪽

2) HEAP
a) 완전 이진 트리의 모습
b) - 최대 힙 : (모든 노드에게 적용) 부모값 > 자식 값
    - 최소 힙: 부모값 < 자식 값
